import 'package:code_aware/view/screens/in_app_screens/contest_page.dart';
...
class MyCustomCard extends State<CustomCard> {
  ...
  Widget showIcon(String site){
    String path = getImagePath(site);
    return Image.asset(path,width: 0);
  }

  String getImagePath(String site){
          ...
          setState(() {
            Navigator.of(context).push(MaterialPageRoute(
                builder: (context) => ContestPage(
                    start: getStart(widget.contestStartTime),
                    time: getTime(widget.contestStartTime),
                    duration: getDuration(widget.duration),
                    status: getStatus(widget.status, widget.in24hrs),
                    url: widget.url,
                    icon: getImagePath(widget.siteName),
                    name: widget.contestName
                )
            ));
          });
        },
