...
class User:
    ...
    def codechef(self):
        ...
        print(rating,max_rating,ranks,global_rank,country_rank)
    ...
    def leetcode(self):
        ...
        if r.status_code!=200:
            raise UsernameError('User not found')
        check = r.html.find('.body > div > div > div > div:nth-child(1) > div > div.text-label-2.dark\:text-dark-label-2.flex.flex-col.space-y-4 > div.flex.space-x-4 > div.flex.flex-col > div.flex.flex-1.items-end.space-x-\[5px\].text-base > span.ttext-label-1.dark\:text-dark-label-1.font-medium').text
        print(check)
        if not len(check):
            print('hh')
            raise UsernameError('User not found')
